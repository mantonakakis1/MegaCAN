...
typedef struct MegaCAN_broadcast_message_t {
  ...
  // offset 12
  float wallfuel1 = 0; // Calculated volume of fuel on intake walls from EAE (in
                       // injector pulsewidth units), us
...
public:
  ...
  const uint32_t getBaseId();
  void setBaseId(const uint32_t baseId);

private:
  const uint32_t _baseId;
};
